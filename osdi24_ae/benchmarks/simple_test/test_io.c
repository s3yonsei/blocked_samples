...
void wrapped_fsync(int fd) {fsync(fd);}
...
void* t_function(void* arg)
{
	...
	sprintf(filename, "/media/nvme/file-%ld", thread_index);
...
int main()
{
	...
	for(i = 0; i < NUM_THREAD; i++) {
		...
		//thr_id[i] = pthread_create(&p_thread[i], &attr, t_function, (void*)i);

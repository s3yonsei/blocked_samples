...
//	printf("KeyMayMatch_Topfilter\n");
  ...
  return MayMatch_Topfilter(key, prefix_extractor, block_offset, no_io, const_ikey_ptr,
                  get_context, lookup_context,
                  &FullFilterBlockReader::KeyMayMatch, AIO, order);
}
...
// CONFIG_AIO
Status PartitionedFilterBlockReader::GetFilterPartitionBlock_AIO(
    FilePrefetchBuffer* prefetch_buffer, const BlockHandle& fltr_blk_handle,
    bool no_io, GetContext* get_context,
    BlockCacheLookupContext* lookup_context,
    CachableEntry<ParsedFullFilterBlock>* filter_block, int order) const {
//	printf("GetFilterPartitionBlock_AIO\n");
  assert(table());
  assert(filter_block);
...
//  start = ((unsigned long long)hi << 32) | lo;
  const Status s =
      table()->RetrieveBlock_AIO(prefetch_buffer, read_options, fltr_blk_handle,
                             UncompressionDict::GetEmptyDict(), filter_block,
                             BlockType::kFilter, get_context, lookup_context,
                             /* for_compaction */ false, /* use_cache */ true,
                             /* wait_for_cache */ true, TOP_FILTER, order);
//  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
//  end = ((unsigned long long)hi << 32) | lo;

//  if(gettid()%NUM_THREADS == 0)	printf("RetrieveBlock order %d %llu\n",order,(end-start)/2600);
  
  //  printf("exit RetrieveBlock_AIO\n");
  return s;
}
...
// CONFIG_AIO
bool PartitionedFilterBlockReader::MayMatch_Topfilter(
    const Slice& slice, const SliceTransform* prefix_extractor,
    uint64_t block_offset, bool no_io, const Slice* const_ikey_ptr,
    GetContext* get_context, BlockCacheLookupContext* lookup_context,
    FilterFunction filter_function, bool *AIO, int order) const {
//	printf("MayMatch_Topfilter\n");
  Status s;
  CachableEntry<Block> filter_block;
  s = GetOrReadFilterBlock(no_io, get_context, lookup_context, &filter_block);
  ...
  s = GetFilterPartitionBlock_AIO(nullptr /* prefetch_buffer */, 
		  						filter_handle, no_io, get_context, lookup_context,
								&filter_partition_block, order);

  if(order == 0){
	  if(s.get_code() == Status::kAIO){
	  	  *AIO = true;
		  return false;
	  }
  }
  ...
  if(order == 2){
	  if(s.get_code() == Status::kNotYet){
		  *AIO = true;
		  return false;
	  }
  }

//  printf("exit GetFilterPartitionBlock_AIO\n");
  if (UNLIKELY(!s.ok())) {
    IGNORE_STATUS_IF_ERROR(s);
    return true;
  }

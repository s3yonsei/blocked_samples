...
std::atomic<int> FILTER_HIT[6], FILTER_MISS[6];
std::atomic<int> INDEX_HIT[6], INDEX_MISS[6];
std::atomic<int> DATA_HIT[6], DATA_MISS[6];

extern struct io_uring *ring;
extern std::vector<free_node> *free_list;
extern int *front;
extern int *rear;
...
Status BlockBasedTable::MaybeReadBlockAndLoadToCache_AIO(
    ...
    BlockContents* contents, int type, int order) const {
//	printf("MaybeReadBlockAndLoadToCache_AIO\n");
		...
		if (!contents) {
			...
			if (block_entry->GetValue() || block_entry->GetCacheHandle()) {
				...
				if(block_type == BlockType::kFilter){
					FILTER_HIT[rep_->level].fetch_add(1);
				}
				if(block_type == BlockType::kIndex){
					INDEX_HIT[rep_->level].fetch_add(1);
				}
				if(block_type == BlockType::kFilter){
					DATA_HIT[rep_->level].fetch_add(1);
				}
...
//	}
    ...
    if (block_entry->GetValue() == nullptr &&
      ...
      if (!contents) {
		...
		BlockFetcher block_fetcher(rep_->file.get(), 
				prefetch_buffer, rep_->footer, ro, handle,
		...
		s = block_fetcher.ReadBlockContents_AIO(type, order);
		
		if(order == 0 || order == 3){
			if(block_type == BlockType::kFilter){
				FILTER_MISS[rep_->level].fetch_add(1);
			}
			if(block_type == BlockType::kIndex){
				INDEX_MISS[rep_->level].fetch_add(1);
			}
			if(block_type == BlockType::kFilter){
				DATA_MISS[rep_->level].fetch_add(1);
			}

			if(top_filter_list[aio_id].back()->read_type == DEVICE ||
					top_filter_list[aio_id].back()->read_type == INDEX_DEVICE){
				s.set_code(Status::kAIO);
				return s;
			}
//			printf("exit ReadBlockContents_AIO 0 %ld\n",gettid());
		}

		if(!s.ok()){
			printf("not ok status %d %d\n", order, aio_id);
		}

//		printf("exit ReadBlockContents_AIO 1 %ld\n",gettid());
...
Status BlockBasedTable::RetrieveBlock_AIO(
    ...
    CachableEntry<TBlocklike>* block_entry, BlockType block_type,
    GetContext* get_context, BlockCacheLookupContext* lookup_context,
    bool for_compaction, bool use_cache, bool wait_for_cache, int type, int order) const {
//	printf("RetrieveBlock_AIO\n");
  assert(block_entry);
  assert(block_entry->IsEmpty());

  Status s;
  if (use_cache) {
    s = MaybeReadBlockAndLoadToCache_AIO(
        ...
        /*contents=*/nullptr, type, order);

	if(s.get_code() == Status::kAIO && order == 0){
		return s;
	}

	if(s.get_code() == Status::kNotYet && order == 2){
		return s;
	}
...
//	printf("exit MaybeReadBlockAndLoadToCache_AIO\n");
    if (!s.ok()) {
      return s;
    }
...
// CONFIG_AIO
template Status BlockBasedTable::RetrieveBlock_AIO<ParsedFullFilterBlock>(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
...
template Status BlockBasedTable::RetrieveBlock_AIO<Block>(
    FilePrefetchBuffer* prefetch_buffer, const ReadOptions& ro,
    const BlockHandle& handle, const UncompressionDict& uncompression_dict,
...
// CONFIG_AIO
bool BlockBasedTable::FullFilterKeyMayMatch_Topfilter(
    FilterBlockReader* filter, const Slice& internal_key, const bool no_io,
    const SliceTransform* prefix_extractor, GetContext* get_context,
    BlockCacheLookupContext* lookup_context, bool *AIO, int order) const {
//  printf("FullFilterKeyMayMatch_Topfilter\n");
  ...
  if (rep_->whole_key_filtering) {
    may_match =
        filter->KeyMayMatch_Topfilter(user_key_without_ts, prefix_extractor, kNotValid,
                            no_io, const_ikey_ptr, get_context, lookup_context, AIO, order);
	if(*AIO == true && order == 0)	
		return false;
	
	if(*AIO == true && order == 2)
		return false;
//	printf("exit KeyMayMatch_Topfilter\n");
...
// CONFIG_AIO
Status BlockBasedTable::Get_Topfilter(const ReadOptions& read_options, const Slice& key,
                            GetContext* get_context,
                            const SliceTransform* prefix_extractor,
                            bool skip_filters,
							int order) {
//	printf("Blockbased Get_Topfilter\n");
...
//  int aio_id = gettid() % NUM_THREADS;
  ...
  if (order == 0 || (top_filter_list[aio_id].front()->buf != nullptr && order == 1)
		  || order == 2){
	  may_match = FullFilterKeyMayMatch_Topfilter(
			  filter, key, no_io, prefix_extractor, get_context, 
			  &lookup_context, &AIO, order);
	  // if AIO == true, io_submit occured
	  if(order == 0){
		  if(AIO == false){
			  assert(top_filter_list[aio_id].back()->buf == nullptr);
			  top_filter_list[aio_id].back()->may_match = may_match;
		  }
		  return s;
	  }
	  if(order == 2){
		  if(AIO == true){	// pre level filter is not match
			  s.set_code(Status::kNotYet);
		  }
		  else{
			  top_filter_list[aio_id].front()->may_match = may_match;
		  }
		  return s;
	  }
  }
  else if(top_filter_list[aio_id].front()->buf == nullptr && order == 1){
	  may_match = top_filter_list[aio_id].front()->may_match;
  }
  else if(order == 3){	// request final level index block
...
//  printf("exit FullFilterKeyMayMatch_Topfilter\n");
  ///////////////////// Filter block ///////////////////////
...
//	  printf("b\n");
	...
	bool is_index_aio = false;
	if((top_filter_list[aio_id].front()->HitFileLevel == top_filter_list[aio_id].back()->HitFileLevel) &&
			(top_filter_list[aio_id].back()->read_type == INDEX_DEVICE ||
			 top_filter_list[aio_id].back()->read_type == INDEX_IO_COMPLETE)){
		is_index_aio = true;
		if(top_filter_list[aio_id].back()->read_type == INDEX_DEVICE){
		// wait for partition index i/o
				...
				if(cqe != nullptr){
					...
					for(int j=0; j < v_size; j++){
						if(top_filter_list[aio_id][j]->read_type != INDEX_DEVICE &&
								top_filter_list[aio_id][j]->read_type != DEVICE) continue;
    ...
    for (is_index_aio == true ? iiter->Seek_AIO(key, 4) : iiter->Seek(key); 
			iiter->Valid() && !done; iiter->Next()) {

...
extern unsigned long long s_2, e_2;
...
//	  printf("ddd\n");
    ...
    } else if (direct_io_buf_.get() != nullptr || aio_flag[gettid()%NUM_THREADS] == 1) {
      if (compression_type_ == kNoCompression) {
        CopyBufferToHeapBuf();
      } else {
//	  printf("fff\n");
...
// CONFIG_AIO
IOStatus BlockFetcher::ReadBlockContents_AIO(int type, int order) {
//	printf("ReadBlockContents_AIO %ld\n",gettid());
  if(order == 0){
    ...
    if (io_status_.ok()) {
		if (file_->use_direct_io()) {
			if(order == 0 || order == 3){
				...
				io_status_ = file_->Read_AIO(
						opts, handle_.offset(), block_size_with_trailer_, &slice_, nullptr,
						&direct_io_buf_, read_options_.rate_limiter_priority, type, order);
...
//		printf("12\n");
	aio_flag[gettid() % NUM_THREADS]=1;
    GetBlockContents();
	aio_flag[gettid() % NUM_THREADS]=0;
  }

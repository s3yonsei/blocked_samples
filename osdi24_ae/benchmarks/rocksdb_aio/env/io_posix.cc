...
//extern io_context_t *ioctx;
//extern struct iocb ***iocb_list;
extern struct io_uring *ring;
extern int *front;
extern unsigned long long init_start;
extern unsigned long long s_1, e_1, io_s, io_e;
extern thread_local int aio_id;

//extern std::list<aio<ParsedFullFilterBlock>> partition_index_list;
...
// CONFIG_AIO
IOStatus PosixRandomAccessFile::Read_TF_AIO(uint64_t offset, size_t n,
                                     const IOOptions& /*opts*/, char* scratch,
...
//  printf("AIO read tid %ld\n",gettid());

/*  io_prep_pread(&(top_filter_list[aio_id].back()->iocb), fd_, ptr, n, static_cast<off_t>(offset));
  iocb_list[aio_id][front[aio_id]] = &(top_filter_list[aio_id].back()->iocb);
*/
/*  unsigned long long lo, hi;
  if(aio_id == 0){
	  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
	  e_1 = ((unsigned long long)hi << 32) | lo;      
	  printf("start to submit %llu\n", (e_1-s_1)/2600);
  }
...
*/
  struct io_uring_sqe *sqe = io_uring_get_sqe(&ring[aio_id]);
  ...
  sqe->user_data = top_filter_list[aio_id].back()->cnt;
  io_uring_prep_readv(sqe, fd_, &iov, 1, static_cast<off_t>(offset));
//  if(aio_id == 0){
//	  printf("insert %llu\n", sqe->user_data);
//  }

  int ret = io_uring_submit(&ring[aio_id]);
/*
  if(aio_id == 0){
	  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
	  end = ((unsigned long long)hi << 32) | lo;
	  printf("submit latency %llu\n", (end-start)/2600);

	  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
	  io_s = ((unsigned long long)hi << 32) | lo;      
  }
*/

  if(top_filter_list[aio_id].back()->read_type == CACHE){
	  top_filter_list[aio_id].back()->read_type = DEVICE;
  }
  else if(top_filter_list[aio_id].back()->read_type == INDEX_CACHE){
	  top_filter_list[aio_id].back()->read_type = INDEX_DEVICE;
  }
  else{
	  printf("sibal error\n");
	  fflush(stdout);
...
/*  unsigned long long end = 0, lo, hi;
  if(aio_id == 0){
	  ...
	  printf("before io_submit cnt %d at %llu\n", top_filter_list[aio_id].back()->cnt, (end-init_start)/2600);
  }
  */
//  unsigned long long start, end, lo, hi;
//  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
//  start = ((unsigned long long)hi << 32)| lo;
//  int ret = io_submit(ioctx[aio_id], 1, &iocb_list[aio_id][front[aio_id]]);
//	if(aio_id == 0)	printf("io_submit\n");
//  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
//  end = ((unsigned long long)hi << 32) | lo;
//  if(aio_id == 0)	printf("io_submit %llu\n", (end-start)/2600);
  
  front[aio_id] = (front[aio_id]+1) % IO_SIZE;
 
  if(ret != 1){
	  //printf("Error in io_submit\n");
	  printf("Error in io_uring_submit\n");
  }
/*
  if(aio_id == 0){
	  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
	  end = ((unsigned long long)hi << 32) | lo;
	  printf("after io_submit cnt %d at %llu\n", top_filter_list[aio_id].back()->cnt, (end-init_start)/2600);
  }
*/
  return s;
}
...
/*
IOStatus PosixRandomAccessFile::Read_PF_AIO(uint64_t offset, size_t n,
                                     const IOOptions&*/ /*opts*//*, char* scratch,
                                     IODebugContext* *//*dbg*//*) const {
  if (use_direct_io()) {
    assert(IsSectorAligned(offset, GetRequiredBufferAlignment()));
  ...
  bzero((char*)&(partition_filter_list.back().aio_node), sizeof(struct aiocb));
  (partition_filter_list.back().aio_node).aio_fildes = fd_;
  (partition_filter_list.back().aio_node).aio_offset = static_cast<off_t>(offset);
  (partition_filter_list.back().aio_node).aio_buf = ptr;
  (partition_filter_list.back().aio_node).aio_nbytes = n;
  (partition_filter_list.back().aio_node).aio_sigevent.sigev_notify = SIGEV_NONE;
  partition_filter_list.back().read_type = DEVICE;
  
  aio_read(&(partition_filter_list.back().aio_node));
  
  return s;
}
*/
/*
IOStatus PosixRandomAccessFile::Read_TI_AIO(uint64_t offset, size_t n,
                                     const IOOptions& *//*opts*//*, char* scratch,
                                     IODebugContext* *//*dbg*//*) const {
  if (use_direct_io()) {
    assert(IsSectorAligned(offset, GetRequiredBufferAlignment()));
  ...
  bzero((char*)&(top_index_list.back().aio_node), sizeof(struct aiocb));
  (top_index_list.back().aio_node).aio_fildes = fd_;
  (top_index_list.back().aio_node).aio_offset = static_cast<off_t>(offset);
  (top_index_list.back().aio_node).aio_buf = ptr;
  (top_index_list.back().aio_node).aio_nbytes = n;
  (top_index_list.back().aio_node).aio_sigevent.sigev_notify = SIGEV_NONE;
  top_index_list.back().read_type = DEVICE;
  
  aio_read(&(top_index_list.back().aio_node));

  return s;
}
*/
/*
IOStatus PosixRandomAccessFile::Read_PI_AIO(uint64_t offset, size_t n,
                                     const IOOptions& *//*opts*//*, char* scratch,
                                     IODebugContext* *//*dbg*//*) const {
  if (use_direct_io()) {
    assert(IsSectorAligned(offset, GetRequiredBufferAlignment()));
  ...
  bzero((char*)&(partition_index_list.back().aio_node), sizeof(struct aiocb));
  (partition_index_list.back().aio_node).aio_fildes = fd_;
  (partition_index_list.back().aio_node).aio_offset = static_cast<off_t>(offset);
  (partition_index_list.back().aio_node).aio_buf = ptr;
  (partition_index_list.back().aio_node).aio_nbytes = n;
  (partition_index_list.back().aio_node).aio_sigevent.sigev_notify = SIGEV_NONE;
  partition_index_list.back().read_type = DEVICE;
  
  aio_read(&(partition_index_list.back().aio_node));
...
// CONFIG_AIO
IOStatus PosixMmapReadableFile::Read_TF_AIO(uint64_t offset, size_t n,
                                     const IOOptions& /*opts*/,
                                     char* /*scratch*/,
                                     IODebugContext* /*dbg*/) const {
	Slice* result = NULL;
...
//IOStatus PosixMmapReadableFile::Read_PF_AIO(uint64_t offset, size_t n,
//                                     const IOOptions& /*opts*/,
//                                     char* /*scratch*/,
//                                     IODebugContext* /*dbg*/) const {
/*	Slice* result = NULL;
...
*/
//IOStatus PosixMmapReadableFile::Read_TI_AIO(uint64_t offset, size_t n,
//                                     const IOOptions& /*opts*/,
//                                     char* /*scratch*/,
//                                     IODebugContext* /*dbg*/) const {
/*	Slice* result = NULL;
...
*/
//IOStatus PosixMmapReadableFile::Read_PI_AIO(uint64_t offset, size_t n,
//                                     const IOOptions& /*opts*/,
//                                     char* /*scratch*/,
//                                     IODebugContext* /*dbg*/) const {
/*	Slice* result = NULL;

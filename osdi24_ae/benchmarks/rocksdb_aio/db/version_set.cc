...
// CONFIG_AIO
#include "aio/myaio.h"
std::vector<aio*> *top_filter_list;
std::vector<free_node> *free_list;
std::list<aio> partition_filter_list;
std::list<aio> top_index_list;
std::list<aio> partition_index_list;
...
int *aio_flag;
int NUM_THREADS = -1;
//std::atomic<int> miss_cnt;
unsigned long long init_start;
int flag;
unsigned long long cnt[64] = {0,};
unsigned long long s_1, s_2, e_1, e_2, io_s, io_e;

extern std::atomic<int> FILTER_HIT[6], FILTER_MISS[6];
extern std::atomic<int> INDEX_HIT[6], INDEX_MISS[6];
extern std::atomic<int> DATA_HIT[6], DATA_MISS[6];
std::atomic<unsigned long long> total_get;
unsigned long long check_get_num = 1000;

thread_local int aio_id = -1;
...
class LevelIterator final : public InternalIterator {
 public:
  ...
  // CONFIG_AIO
  void Seek_AIO(const Slice& /*target*/, int /*order*/) override {
	  return;
  }
...
//  struct io_event events[IO_SIZE];
//  memset(events, 0, sizeof(struct io_event)*IO_SIZE);
...
//  NUM_THREADS = cfd_->maxClientThreads();
  ...
  top_filter_list = cfd_->get_top_filter_list();
  free_list = cfd_->get_free_list();
//  ioctx = cfd_->get_ioctx();

  ring = cfd_->get_ring();
  
//  iocb_list = cfd_->get_iocb_list();
  front = cfd_->get_front();
  rear = cfd_->get_rear();
  aio_flag = cfd_->get_aio_flag();
...
//	  printf("\nGet start\n");
//	  asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
//	  init_start = ((unsigned long long)hi << 32) | lo;
//  }

//  unsigned long long lo, hi;
  int index_aio_flag = 0;
  while (1) {
	if(f == nullptr && top_filter_list[aio_id].empty()){
		break;
	}

	// request final level index block
	if(f == nullptr && top_filter_list[aio_id].empty() == false 
			&& index_aio_flag == 0 && ((front[aio_id] + 1) % IO_SIZE != rear[aio_id])) {
		index_aio_flag = 1;

		aio *tf_aio = (aio*)malloc(sizeof(aio));
		tf_aio->read_type = INDEX_CACHE;
		tf_aio->timer_enabled = top_filter_list[aio_id].back()->timer_enabled;
		tf_aio->HitFileLevel = top_filter_list[aio_id].back()->HitFileLevel;
		tf_aio->HitFileLastInLevel = top_filter_list[aio_id].back()->HitFileLastInLevel;
		tf_aio->file_meta = top_filter_list[aio_id].back()->file_meta;
		tf_aio->buf = nullptr;
		tf_aio->bufstart = nullptr;
		tf_aio->ok = false;
		tf_aio->may_match = true;
		tf_aio->cnt = cnt[aio_id]++;

		top_filter_list[aio_id].emplace_back(tf_aio);

		*status = table_cache_->Get_Topfilter(
				read_options, *internal_comparator(), *tf_aio->file_meta, ikey,
				&get_context, mutable_cf_options_.prefix_extractor,
				cfd_->internal_stats()->GetFileReadHist(tf_aio->HitFileLevel),
				IsFilterSkipped(static_cast<int>(tf_aio->HitFileLevel),
					tf_aio->HitFileLastInLevel),
				tf_aio->HitFileLevel, max_file_size_for_l0_meta_pin_, 3);
	}

	// 1. Insert top filter aio_block
	if(f != nullptr && ((front[aio_id] + 1) % IO_SIZE != rear[aio_id])){
		...
		//aio *tf_aio = (aio*)rocksdb::port::cacheline_aligned_alloc(sizeof(aio));
		aio *tf_aio = (aio*)malloc(sizeof(aio));
		tf_aio->read_type = CACHE;
		tf_aio->timer_enabled = timer_enabled;
		tf_aio->HitFileLevel = fp.GetHitFileLevel();
		tf_aio->HitFileLastInLevel = fp.IsHitFileLastInLevel();
		tf_aio->file_meta = f->file_metadata;
		tf_aio->buf = nullptr;
		tf_aio->bufstart = nullptr;
		tf_aio->ok = false;
		tf_aio->may_match = false;
		tf_aio->cnt = cnt[aio_id]++;
		...
		*status = table_cache_->Get_Topfilter(
				...
				fp.GetHitFileLevel(), max_file_size_for_l0_meta_pin_, 0);
	...
	while(1){
		// fill cache
		if(top_filter_list[aio_id].front()->read_type == DEVICE){
			struct io_uring_cqe *cqe = nullptr;
//			if(f == nullptr){
//				io_uring_wait_cqe(&ring[aio_id], &cqe);
//			}
//			else{
				io_uring_peek_cqe(&ring[aio_id], &cqe);
//			}
			if(cqe != nullptr){
				rear[aio_id] = (rear[aio_id] + 1) % IO_SIZE;
				int v_size = top_filter_list[aio_id].size();
				int tmp_flag = 0;
				for(int j=0; j < v_size; j++){
					if(top_filter_list[aio_id][j]->read_type != DEVICE
							&& top_filter_list[aio_id][j]->read_type != INDEX_DEVICE)	continue;
					if(top_filter_list[aio_id][j]->cnt == cqe->user_data){
						if(top_filter_list[aio_id][j]->read_type == DEVICE){
							top_filter_list[aio_id][j]->read_type = IO_COMPLETE;
						}
						if(top_filter_list[aio_id][j]->read_type == INDEX_DEVICE){
							top_filter_list[aio_id][j]->read_type = INDEX_IO_COMPLETE;
						}
						tmp_flag = 1;
						break;
					}
				...
				if(tmp_flag == 0){
					int f_size = free_list[aio_id].size();
					for(int j=f_size-1; j>=0; j--){
						if(free_list[aio_id][j].cnt == cqe->user_data){
//							if(aio_id == 0){
//								if(free_list[aio_id][j].buf == nullptr){
//									printf("free error\n");
//								}
//								printf("real free %llu\n", cqe->user_data);
//							}
							free(free_list[aio_id][j].buf);
							free_list[aio_id][j].buf = nullptr;
							free_list[aio_id].erase(free_list[aio_id].begin()+j);
							break;
						}
					}
				}
				io_uring_cqe_seen(&ring[aio_id], cqe);
			}
//			io_uring_cqe_seen(&ring[aio_id], cqe);
//			int	receiveNum = io_getevents(ioctx[aio_id], 0, IO_SIZE, events, &timeout);
/*			if(receiveNum == 0){
					unsigned int HitFileLevel = top_filter_list[aio_id].front()->HitFileLevel;
					*status = table_cache_->Get_Topfilter(
							read_options, *internal_comparator(), 
							*(top_filter_list[aio_id].front()->file_meta), ikey,
							&get_context, mutable_cf_options_.prefix_extractor,
							cfd_->internal_stats()->GetFileReadHist(HitFileLevel),
							IsFilterSkipped(static_cast<int>(HitFileLevel), 
								top_filter_list[aio_id].front()->HitFileLastInLevel),
							HitFileLevel, max_file_size_for_l0_meta_pin_, 2);
					if((*status).get_code() != Status::kNotYet){
						top_filter_list[aio_id].front()->read_type = IO_COMPLETE;
						top_filter_list[aio_id].front()->buf = nullptr;
					}
			}
			else{*/
/*			rear[aio_id] = (rear[aio_id] + receiveNum) % IO_SIZE;
			for(int i=0; i<receiveNum; i++){
				int v_size = top_filter_list[aio_id].size();
				int tmp_flag = 0;
				for(int j=0; j < v_size; j++){
					if(top_filter_list[aio_id][j]->read_type != DEVICE)	continue;
					if(&(top_filter_list[aio_id][j]->iocb) == (struct iocb*)events[i].obj){
						top_filter_list[aio_id][j]->read_type = IO_COMPLETE;
						tmp_flag = 1;
						break;
					}
				...
				if(tmp_flag == 0){
					int f_size = free_list[aio_id].size();
					for(int j=f_size-1; j>=0; j--){
						if(free_list[aio_id][j].iocb == (struct iocb*)events[i].obj){
							free(free_list[aio_id][j].buf);
							free_list[aio_id].erase(free_list[aio_id].begin()+j);
							break;
						}
		...
		// complete aio_read || already cache
		if((top_filter_list[aio_id].front()->read_type == IO_COMPLETE)
							|| (top_filter_list[aio_id].front()->read_type == CACHE)) {
...
/*			if(aio_id == 0){
				...
				if(top_filter_list[aio_id].front()->read_type == IO_COMPLETE)
					printf("IO_COMPLETE latency: %llu cnt %d\n", 
							(end - init_start)/2600, top_filter_list[aio_id].front()->cnt);
				if(top_filter_list[aio_id].front()->read_type == CACHE)
					printf("CACHE latency: %llu cnt %d\n", 
							(end - init_start)/2600, top_filter_list[aio_id].front()->cnt);
			}
*/
//			if(aio_id == 0){
//				asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
//				s_2 = ((unsigned long long)hi << 32) | lo;
//			}
			*status = table_cache_->Get_Topfilter(
					read_options, *internal_comparator(), 
					*(top_filter_list[aio_id].front()->file_meta), ikey,
					...
					IsFilterSkipped(static_cast<int>(HitFileLevel), 
						top_filter_list[aio_id].front()->HitFileLastInLevel),
					HitFileLevel, max_file_size_for_l0_meta_pin_, 1);
			assert(status->ok());
/*			if(aio_id == 0){
				asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
				end = ((unsigned long long)hi << 32) | lo;
				printf("Get_2 cnt %d latency %llu\n", top_filter_list[aio_id].front()->cnt, (end-init_start)/2600);
			}
*/			
			if(top_filter_list[aio_id].front()->read_type == IO_COMPLETE){
				free(top_filter_list[aio_id].front()->buf);
				top_filter_list[aio_id].front()->buf = nullptr;
			}

			StopWatchNano timer(clock_, top_filter_list[aio_id].front()->timer_enabled /* auto_tart */);

			// TODO: examine the behavior for corrupted key
			if (top_filter_list[aio_id].front()->timer_enabled) {
				PERF_COUNTER_BY_LEVEL_ADD(get_from_table_nanos, timer.ElapsedNanos(), HitFileLevel);
			}
			...
			int tf_size = top_filter_list[aio_id].size();
...
/*					if(aio_id % NUM_THREADS == 0){
						unsigned long long now_get = total_get.load();
						if(now_get > check_get_num){
							check_get_num += 1000;
							printf("now get count %lld\n", now_get);
							for(int i=0;i<5;i++){
								printf("Filter level %d Hit %d Miss %d\n", i, FILTER_HIT[i].load(), FILTER_MISS[i].load());
//								printf("Index level %d Hit %d Miss %d\n", i, INDEX_HIT[i].load(), INDEX_MISS[i].load());
//								printf("Data level %d Hit %d Miss %d\n\n", i, DATA_HIT[i].load(), DATA_MISS[i].load());
							}
							printf("\n");
						}
					}*/
//					for(int i=0;i<tf_size;i++){
//						if(top_filter_list[aio_id][i]->buf != nullptr){
//							int ret_can = io_cancel(ioctx[aio_id], &(top_filter_list[aio_id][i]->iocb), nullptr);
//							if(ret_can == 0){
...
//						asm volatile("rdtsc" : "=a" (lo), "=d" (hi));
//						end = ((unsigned long long)hi << 32) | lo;
//						printf("FOUND total latency: %llu\n", (end-init_start)/2600);
//					}
//					for(std::list<aio*>::iterator iter=top_filter_list[aio_id].begin();
//							iter != top_filter_list[aio_id].end(); iter++){
//						if((*iter)->read_type == DEVICE){
//							int c_ret = io_cancel(ioctx[aio_id], &((*iter)->iocb), events);
//							if(c_ret != 0){
...
//						}
//						if((*iter)->buf != nullptr){
//							fprintf(of[aio_id],"size %ld count %d delete %p\n",
//									top_filter_list[aio_id].size(),
//									t_order[aio_id],
//									(*iter)->buf);
//							delete[] ((*iter)->buf);
//							(*iter)->buf = nullptr;
...
//					}
//					fclose(of[gettid()%8]);
					if(top_filter_list[aio_id].back()->read_type == INDEX_IO_COMPLETE){
						free(top_filter_list[aio_id].back()->buf);
						top_filter_list[aio_id].back()->buf = nullptr;
					}

					if (HitFileLevel == 0) {
						RecordTick(db_statistics_, GET_HIT_L0);
					} else if (HitFileLevel == 1) {
						RecordTick(db_statistics_, GET_HIT_L1);
					} else if (HitFileLevel >= 2) {
						RecordTick(db_statistics_, GET_HIT_L2_AND_UP);
					}

					PERF_COUNTER_BY_LEVEL_ADD(user_key_return_count, 1, HitFileLevel);
					if (is_blob_index) {
						if (do_merge && value) {
...
//					for (std::list<aio*>::iterator iter = top_filter_list[aio_id].begin(); iter != top_filter_list[aio_id].end(); iter++)
//					{
//						printf("%p\n", (*iter)->buf);
//						if((*iter)->buf != nullptr){
//							delete[] (*iter)->buf;
...
//					}
					...
					for(int i=0; i<tf_size; i++){
						if(top_filter_list[aio_id][i]->read_type == DEVICE ||
								top_filter_list[aio_id][i]->read_type == INDEX_DEVICE){
							struct free_node tmp_node;
							//tmp_node.iocb = &(top_filter_list[aio_id][i]->iocb);
							tmp_node.cnt = top_filter_list[aio_id][i]->cnt;
//							if(aio_id == 0){
//								printf("insert free %llu\n", tmp_node.cnt);
//							}
							tmp_node.buf = top_filter_list[aio_id][i]->buf;
							free_list[aio_id].emplace_back(tmp_node);
						}
//						free(top_filter_list[aio_id][i]);
					}
...
//					printf("%d\n",miss_cnt.load());
			...
			top_filter_list[aio_id].erase(top_filter_list[aio_id].begin());
//			std::list<aio*>::iterator iter = top_filter_list[aio_id].begin();
//			top_filter_list[aio_id].erase(iter);

			if(top_filter_list[aio_id].empty())	break;
		}
		else	break;
	}

/*
	// 2. Insert partition filter aio_block
	for(tf_iter = top_filter_list.begin(); tf_iter != top_filter_list.end(); tf_iter++){
		if((!partition_filter_list.empty()) && ((*tf_iter).level == (*pf_iter).level)){
			pf_iter++;
			continue;
		}
		// fill cache
		if(aio_return(&((*tf_iter).aio_node)) > 0){
			BlockBasedTable kk((*tf_iter).rep_, nullptr);
			const UncompressionDict& uncompression_dict = UncompressionDict::GetEmptyDict();
			kk.PutDataBlockToCache_AIO(
			(*tf_iter).key, 
			(*tf_iter).block_cache, 
			(*tf_iter).block_cache_compressed,
			(*tf_iter).block_entry, 
			(*tf_iter).contents, 
			(*tf_iter).raw_block_comp_type,
			uncompression_dict, 
			GetMemoryAllocator((*tf_iter).rep_->table_options),
			(*tf_iter).block_type, 
			(*tf_iter).get_context);
		}	

		// read partition filter
		if(aio_return(&((*tf_iter).aio_node)) > 0 || (*tf_iter).read_type == CACHE){
			aio<ParsedFullFilterBlock> pf_aio;
			pf_aio.aio_f = (*tf_iter).aio_f;
			pf_aio.level = (*tf_iter).level;
			pf_aio.read_type = CACHE;
			pf_aio.timer_enabled = (*tf_iter).timer_enabled;
			partition_filter_list.push_back(pf_aio);
	...
	// 3. Insert top index aio_block
	for(pf_iter = partition_filter_list.begin(); pf_iter != partition_filter_list.end(); pf_iter++){
		if((!top_index_list.empty()) && ((*pf_iter).level == (*ti_iter).level)){
			ti_iter++;
			continue;
		}
		
		if(aio_return(&((*pf_iter).aio_node)) > 0 || (*pf_iter).read_type == CACHE){
			aio<ParsedFullFilterBlock> ti_aio;
			ti_aio.aio_f = (*pf_iter).aio_f;
			ti_aio.level = (*pf_iter).level;
			ti_aio.read_type = CACHE;
			ti_aio.timer_enabled = (*pf_iter).timer_enabled;
			partition_filter_list.push_back(ti_aio);
	...
	// 4. Insert partition index aio_block
	for(ti_iter = top_index_list.begin(); ti_iter != top_index_list.end(); ti_iter++){
		if((!partition_index_list.empty()) && ((*ti_iter).level == (*pi_iter).level)){
			pi_iter++;
			continue;
		}

		if(aio_return(&((*ti_iter).aio_node)) > 0 || (*ti_iter).read_type == CACHE){
			aio<ParsedFullFilterBlock>  pi_aio;
			pi_aio.aio_f = (*ti_iter).aio_f;
			pi_aio.level = (*ti_iter).level;
			pi_aio.read_type = CACHE;
			pi_aio.timer_enabled = (*ti_iter).timer_enabled;
			partition_filter_list.push_back(pi_aio);
	...
	for(pi_iter = partition_index_list.begin(); pi_iter != partition_index_list.end(); pi_iter++){
		if(aio_return(&((*pi_iter).aio_node)) > 0 || (*pi_iter).read_type == CACHE){

			top_filter_list.pop_front();
			partition_filter_list.pop_front();
			top_index_list.pop_front();
			partition_index_list.pop_front();
		}
		else	break;
	}
*/

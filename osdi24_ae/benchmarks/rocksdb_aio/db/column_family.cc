...
ColumnFamilyData::ColumnFamilyData(
  ...
  top_filter_list = new std::vector<aio*>[max_client_threads];
  free_list = new std::vector<free_node>[max_client_threads];
  ioctx = new io_context_t[max_client_threads];
  ring = new struct io_uring[max_client_threads];
  front = new int[max_client_threads];
  rear = new int[max_client_threads];
  iocb_list = new struct iocb**[max_client_threads];
  aio_flag = new int[max_client_threads];

  // initalize iocb & io_setup
  for(int i=0; i<max_client_threads; i++){
	  ioctx[i] = 0;
	  front[i] = 0;
	  rear[i] = 0;
	  aio_flag[i]=0;
	  iocb_list[i] = new struct iocb*[IO_SIZE];
	  
	  if(io_uring_queue_init(IO_SIZE, &ring[i], 0) < 0){
		  printf("io_uring error\n");
	  }
	  
//	  if(io_setup(IO_SIZE, &ioctx[i])<0){

			     ...
			     struct addr_location *al)
{
	...
	if ((al->blocked == PERF_RECORD_MISC_IOWAIT) && perf_host)
		al->level = 'I';
	else if ((al->blocked == PERF_RECORD_MISC_SCHED) && perf_host) 
		al->level = 'S';
	else if ((al->blocked == PERF_RECORD_MISC_LOCKWAIT) && perf_host)
		al->level = 'L';
	else if ((al->blocked == PERF_RECORD_MISC_BLOCKED) && perf_host)
		al->level = 'B';
	else
		al->level = 0;
	/*if ((al->blocked & (PERF_RECORD_MISC_BLOCKED | PERF_RECORD_MISC_LOCKWAIT)) && perf_host) {
		al->level = 'B';
	} else if ((al->blocked == PERF_RECORD_MISC_IOWAIT) && perf_host) {
		al->level = 'I';		
	} else if ((al->blocked == PERF_RECORD_MISC_SCHED) && perf_host) {
		al->level = 'S';	
	} //else if ((al->blocked == PERF_RECORD_MISC_LOCKWAIT) && perf_host) {
	...
	if (cpumode == PERF_RECORD_MISC_KERNEL && perf_host) {
		if (!al->blocked)	al->level = 'k';
		mg = &machine->kmaps;
		load_map = true;
	} else if (cpumode == PERF_RECORD_MISC_USER && perf_host) {
		if (!al->blocked)	al->level = '.';
	} else if (cpumode == PERF_RECORD_MISC_GUEST_KERNEL && perf_guest) {
		if (!al->blocked)	al->level = 'g';
		mg = &machine->kmaps;
		load_map = true;
	} else if (cpumode == PERF_RECORD_MISC_GUEST_USER && perf_guest) {
		if (!al->blocked)	al->level = 'u';
	} else {
		if (!al->blocked)	al->level = 'H';
		al->map = NULL;

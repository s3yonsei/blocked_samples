...
void coz_progress_(void) {
	COZ_INCREMENT_COUNTER(COZ_COUNTER_TYPE_THROUGHPUT, "ep.f90:164");
    /*static unsigned char _initialized = 0;
    static coz_counter_t* _counter = 0;
    
	if(!_initialized) {
      _counter = _call_coz_get_counter(COZ_COUNTER_TYPE_THROUGHPUT, "ep.f90:164");
      _initialized = 1;
    }
    if(_counter) {
      __atomic_add_fetch(&_counter->count, 1, __ATOMIC_RELAXED);
...
/*void coz_progress_() {
  void *p = dlsym(RTLD_DEFAULT, "_coz_progress_");
  static coz_get_counter_t fn;

  memcpy(&fn, &p, sizeof(p));

  fn(COZ_COUNTER_TYPE_THROUGHPUT, __FILE__ ":" STR(__LINE__));
}*/

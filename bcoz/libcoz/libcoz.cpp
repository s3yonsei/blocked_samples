...
void init_coz(void) {
  ...
  char based_blocked;
  stringstream(getenv_safe("COZ_BASED_BLOCKED", "n")) >> based_blocked;

  char blocked_scope;
  stringstream(getenv_safe("COZ_BLOCKED_SCOPE", "n")) >> blocked_scope;
  print_log = getenv("COZ_LOG");
  ...
  profiler::get_instance().startup(output_file,
                    fixed_line.get(),
                    fixed_speedup,
                    end_to_end, blocked_scope, print_log, based_line.get(), based_blocked, based_speedup);
...
static int wrapped_main(int argc, char** argv, char** env) {
  ...
  startup = get_time() - start_time;

  // Run the real main function
  int result = real_main(argc, argv, env);

  real_main_time = get_time() - startup - start_time;
  ...
  end_to_end_time = get_time() - start_time;

  cout << "Overhead breakdown. Startup: " << startup << " real_main_time: " << real_main_time << " end-to-end: " << end_to_end_time << endl;
...
extern "C" int coz_libc_start_main(main_fn_t main_fn, int argc, char** argv,
  ...
  INFO << "Start find the real __libc_start_main";
  auto real_libc_start_main = (decltype(__libc_start_main)*)dlsym(RTLD_NEXT, "__libc_start_main");

  if (real_libc_start_main == NULL)	INFO << "dlsym __libc_start_main failed!";
...
extern "C" {
  ...
  int pthread_spin_lock(pthread_spinlock_t* spinlock) throw() {
	if(initialized)	profiler::get_instance().pre_block();
	int result = real::pthread_spin_lock(spinlock);
	if(initialized)	profiler::get_instance().post_block(true);

	return result;
  }

  int pthread_spin_unlock(pthread_spinlock_t* spinlock) throw() {
	if(initialized)	profiler::get_instance().catch_up();
	return real::pthread_spin_unlock(spinlock);
  }

...
static void* get_pthread_handle() {
  if(pthread_handle == NULL && !__atomic_exchange_n(&in_dlopen, true, __ATOMIC_ACQ_REL)) {
    pthread_handle = dlopen("libpthread-2.30.so", RTLD_NOW | RTLD_GLOBAL);
	if (pthread_handle == NULL)	while(1);
...
static int resolve_fdatasync(int fd) {
  GET_SYMBOL(fdatasync);
  if (real_fdatasync)	return real_fdatasync(fd);
  else return -1;
}

static int resolve_fsync(int fd) {
  GET_SYMBOL(fsync);
  if (real_fsync)	return real_fsync(fd);
  else return -1;
}

static int resolve_nanosleep(const struct timespec *req, struct timespec *rem) {
  GET_SYMBOL(nanosleep);
  if (real_nanosleep)	return real_nanosleep(req, rem);
  else return -1;
}

static ssize_t resolve_pread(int fd, void *buf, size_t count, __off_t pos) {
  GET_SYMBOL(pread);
  if (real_pread)	return real_pread(fd, buf, count, pos);
  else return -1;
}

static void *resolve_mmap(void *mmap_addr, size_t length, int prot, int flags, int fd, __off_t offset) {
  GET_SYMBOL(mmap);
  if (real_mmap)	return real_mmap(mmap_addr, length, prot, flags, fd, offset);
  else return NULL;
}

static ssize_t resolve_pwrite(int fd, const void *buf, size_t count, __off_t pos) {
  GET_SYMBOL(pwrite);
  if (real_pwrite)	return real_pwrite(fd, buf, count, pos);
  else return -1;
}

static ssize_t resolve_write(int fd, const void *buf, size_t count) {
  GET_SYMBOL(write);
  if (real_write)	return real_write(fd, buf, count);
  else return -1;
}

static ssize_t resolve_read(int fd, void *buf, size_t count) {
  GET_SYMBOL(read);
...
static int resolve_pthread_spin_lock(pthread_spinlock_t* spinlock) throw() {
  GET_SYMBOL_HANDLE(pthread_spin_lock, get_pthread_handle());
  if(real_pthread_spin_lock)	return real_pthread_spin_lock(spinlock);
  else return 0;
}

static int resolve_pthread_spin_unlock(pthread_spinlock_t* spinlock) throw() {
  GET_SYMBOL_HANDLE(pthread_spin_unlock, get_pthread_handle());
  if(real_pthread_spin_unlock)	return real_pthread_spin_unlock(spinlock);
  else return 0;
}
